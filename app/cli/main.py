...
@data_app.command("funding")
def data_funding(
    stocks: str = typer.Option("", "--stocks", help="只更新指定股票，逗号分隔"),
    max_stocks: int = typer.Option(0, "--max-stocks", help="最多更新N只上市股票(0=全部)"),
    batch_size: int = typer.Option(50, "--batch-size", help="每批N只后冷却(默认50)"),
    batch_cooldown: int = typer.Option(30, "--batch-cooldown", help="批间冷却秒数(默认30)"),
    check_only: bool = typer.Option(False, "--check-only", help="只显示覆盖/队列状态，不抓取"),
) -> None:
    """融资事件低频更新（数据补全：分红融资比指标的数据前置）

    更新 funding_events（IPO 首发 CNINFO + 增发/配股 东财 F10）。
    - 单股事务原子替换；失败保留旧值并写入独立 retry/missing
    - 防封纪律：东财仅 emweb host，限速 ≤2 req/s，批 50 + 冷却 30s
    - 有界续传：--max-stocks 每轮只处理前 N 只未覆盖股票
    """
    from app.cli.protocol import make_response
    from app.core.funding import FundingUpdater

    _, duck, sqlite = _database_context()
    updater = FundingUpdater(duck=duck, sqlite=sqlite)
    if check_only:
        report = updater.status_report()
    elif stocks.strip():
        codes = [code.strip() for code in stocks.split(",") if code.strip()]
...
@data_app.command("buyback")
def data_buyback(
    check_only: bool = typer.Option(False, "--check-only", help="只显示回购域状态，不抓取"),
) -> None:
    """回购/注销事件低频全量更新（2026-08-26，分红融资比广义分红数据补齐）

    东财股票回购明细单次全市场请求，原子替换 buyback_events。
    """
    from app.cli.protocol import make_response
