        ...
        *,
        ...
        if cross_only:
            ...
            if result.metadata.error:
                ...
                fallback_result = self.adapter.fetch(FetchRequest(
                    data_type="share_capital_history",
                    stock_codes=[stock_code],
                ...
                main_data = fallback_result.data
                fallback_used = True
            else:
        ...
        cross_status = "fallback_eastmoney" if fallback_used else "unverified"
        cross_error: str | None = None
        if fallback_used:
            # 降级主链本身来自东财，不再重复请求东财交叉校验。
            pass
        elif cross_check and self._has_eastmoney_cross_source(stock_code):
            ...
            conn.executemany(
                ...
                [
                    [
                        ...
                        row.get("verified"), row.get("source", "cninfo_capital"),
                        raw_hash, batch_id,
                    ]
                    for row in rows
                ],
