...
def _reset_cache(monkeypatch) -> None:
    monkeypatch.setattr(data_status, "_SUMMARY_CACHE", {"at": 0.0, "data": None})
...
def test_summary_succeeds_without_lock_and_populates_cache(
    ...
    assert data_status._SUMMARY_CACHE["data"] is not None
...
def test_summary_reuses_cache_until_ttl_expires(
    ...
    assert data_status._cached_summary() is not None
    # TTL 内直接复用（无锁也命中缓存）
    ...
    # TTL 过期后重新计算
    monkeypatch.setattr(data_status, "_SUMMARY_CACHE", {"at": 0.0, "data": None})
    assert data_status._cached_summary() is None
