...
const groupOptions = computed(() => [
  { label: '全部分组', value: '' },
  ...groups.value.map((g: any) => ({ label: `${g.group_name} (${g.cnt})`, value: g.group_name })),
])
...
const tableColumns = computed(() => {
  ...
  const colDefs: Record<string, any> = {
    ...
    name: { title: '名称', key: 'name', width: 100, render: (r: any) => r.name || '—' },
    exchange: { title: '交易所', key: 'exchange', width: 70, render: (r: any) => r.exchange || '—' },
    group_name: { title: '分组', key: 'group_name', width: 90 },
    latest_close: { title: '收盘价', key: 'latest_close', width: 80, sorter: 'default' as const, render: (r: any) => r.latest_close != null ? r.latest_close.toFixed(2) : '—' },
    pe_ttm: { title: 'PE', key: 'pe_ttm', width: 70, sorter: 'default' as const, render: (r: any) => r.pe_ttm != null ? r.pe_ttm.toFixed(1) : '—' },
    pb_mrq: { title: 'PB', key: 'pb_mrq', width: 70, sorter: 'default' as const, render: (r: any) => r.pb_mrq != null ? r.pb_mrq.toFixed(2) : '—' },
    roe: { title: 'ROE', key: 'roe', width: 80, sorter: 'default' as const, render: (r: any) => r.roe != null ? (r.roe * 100).toFixed(2) + '%' : '—' },
    gross_margin: { title: '毛利率', key: 'gross_margin', width: 80, sorter: 'default' as const, render: (r: any) => r.gross_margin != null ? (r.gross_margin * 100).toFixed(2) + '%' : '—' },
    net_margin: { title: '净利率', key: 'net_margin', width: 80, sorter: 'default' as const, render: (r: any) => r.net_margin != null ? (r.net_margin * 100).toFixed(2) + '%' : '—' },
    debt_ratio: { title: '负债率', key: 'debt_ratio', width: 80, sorter: 'default' as const, render: (r: any) => r.debt_ratio != null ? (r.debt_ratio * 100).toFixed(2) + '%' : '—' },
    revenue_yoy: { title: '营收YoY', key: 'revenue_yoy', width: 90, sorter: 'default' as const, render: (r: any) => r.revenue_yoy != null ? (r.revenue_yoy * 100).toFixed(2) + '%' : '—' },
    source: { title: '来源', key: 'source', width: 80, render: (r: any) => r.source_result_id ? '筛选' : (r.source_rule_id ? '规则' : '手动') },
  }
  for (const col of selectedColumns.value) {
    if (colDefs[col]) cols.push(colDefs[col])
  }
  cols.push({
...
async function removeStock(code: string, group: string) {
  try {
    ...
    fetchWatchlist()
  } catch (e: any) { message.error(`移除失败: ${e.message}`) }
}
