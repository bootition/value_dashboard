...
const indicatorOptions = [
  ...
  { label: '总市值(亿)', value: 'total_market_cap' },
...
describe('RuleConditionRow 全指标单位显示与换算', () => {
  it('总市值：输入框旁显示“亿”，输入 1000 时发出底层元值 1e11', async () => {
    const wrapper = mountRow(makeCondition({ field: 'total_market_cap', value: 2e10 }))
    expect(wrapper.text()).toContain('亿')
    const numberInput = wrapper.findComponent({ name: 'InputNumber' })
    if (numberInput.exists()) {
      await numberInput.vm.$emit('update:value', 1000)
      const emitted = wrapper.emitted('update:value')
      expect(emitted).toBeTruthy()
      expect(emitted![emitted!.length - 1][0]).toBe(1e11)
    }
  })

  it('总市值：回显时按亿显示（2e10 元 → 200）', () => {
    const wrapper = mountRow(makeCondition({ field: 'total_market_cap', value: 2e10 }))
    expect(wrapper.text()).toContain('亿')
    expect(wrapper.props('condition').value).toBe(2e10)
  })

  it('切换指标后单位随字段变化：市盈率→倍、股息率→%、总市值→亿', async () => {
    const wrapper = mountRow(makeCondition({ field: 'pe_ttm', value: 15 }))
    expect(wrapper.text()).toContain('倍')
    await wrapper.setProps({ condition: makeCondition({ field: 'dividend_yield', value: 0.0529 }) })
    expect(wrapper.text()).toContain('%')
    await wrapper.setProps({ condition: makeCondition({ field: 'total_market_cap', value: 1e11 }) })
    expect(wrapper.text()).toContain('亿')
    expect(wrapper.text()).not.toContain('倍')
  })
})
