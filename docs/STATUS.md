...
- **最后更新**：2026-09-01
- **更新人**：opencode 会话（2026-09-01 更新过程内存治理：`reports/96`）
## 当前裁决（Verdict）
| 层面 | 状态 | 依据 |
|---|---|---|
...
14. **更新内存峰值与启动核对峰值（2026-09-01 更新）**：`raw_response_archive` 新行提交会触发 DuckDB 整表主键校验扫描，财务明细回填因此周期性冲到 18~28GB；已由 `reports/96` 以 schema v16 冷热分层修复。启动数据就绪冷核对仍会短时使用 DuckDB 默认约 25GB memory_limit（观察 15~21GB 后回落），读连接有界 `memory_limit` 评估列入待办。
## 进行中的工作
