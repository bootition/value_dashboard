...
- **最后更新**：2026-08-10
- **更新人**：opencode 会话（个股研究工作台 P1-P4 全部实施通过）
## 当前裁决（Verdict）
| 层面 | 状态 | 依据 |
|---|---|---|
...
| 国债曲线与利差 P3 | ✅ 财政部曲线独立域、TTM已实施股息率与9期限利差、详情国债比较、筛选字段与导出溯源已实施；年度XLSX入口404改走官方单期限历史接口（偏离已记录）；S1 538、前端门禁全绿 | `reports/71`（2026-08-10） |
| 历史股本链与统计 P4 | ✅ CNINFO主链+东财交叉核验20/18可用、PE/PB/TTM股息率/利差历史序列、分位/z-score统计域与筛选已实施；S1 552、前端门禁全绿 | `reports/72`（2026-08-10） |
...
- **P1-P4 新域正式库推进**：业务概览、国债曲线、历史股本链有界续传；统计域按输入指纹原子重建。
- **待办**：CNINFO 分红 ex_date 修复评估（见已知缺口 #4）。
- **已完结**：P1 详情研究工作台、P2 业务概览、P3 国债曲线与利差、P4 历史股本链与统计域均已实施并通过门禁。
...
| `docs/reports/71_TREASURY_CURVE_P3_2026-08-10.md` | **国债曲线与利差 P3**：来源门禁、独立基准域、TTM股息率利差与详情/筛选接入 | **当前国债基准实施依据** |
| `docs/reports/72_CAPITAL_HISTORY_AND_STATISTICS_P4_2026-08-10.md` | **历史股本链与统计 P4**：CNINFO主链/东财交叉核验、PE/PB/TTM股息率/利差序列、分位/z-score统计域与筛选 | **当前历史股本与统计实施依据** |
