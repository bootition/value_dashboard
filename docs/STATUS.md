...
- **最后更新**：2026-09-03
- **更新人**：opencode 会话（2026-09-03 性能连接冲突与 CLI 状态查询修复：`reports/104`；2026-09-02 技术债补全与 UI 修复：`reports/103`）
## 当前裁决（Verdict）
| 层面 | 状态 | 依据 |
|---|---|---|
...
| 全局数据路径复审计与指标输入链 | ✅ **修复完成（2026-08-31）**：回购域接入自动更新；融资/国债/失败重试全部前移到指标快照之前；股本变化与财务 partial 解耦；回购空响应保旧值；国债与统计域指纹升级为内容级；财务明细缺口改用跨行业通用探测字段（银行/券商不再占住回填队头）；写类 CLI 统一单写者锁。定向回归 105+ passed | `reports/92`（2026-08-31） |
| 更新完成后全量复审计 | ✅ **修复完成（2026-08-31）**：更新成功且 ready=true/warning_codes=[]；新修复历史股本链队头重复、国债非交易日 missing 重试、CNINFO 源级 retry 清理、missing_list 冲突告警、既有 S1 测试种子日期矛盾。回归 104+17 passed | `reports/93`（2026-08-31） |
| 数据与代码双重复查 | ✅ **完成（2026-08-31）**：核心链路数据完整；修复业务概览 CLI 加速命令重复刷前缀、PriceBackfiller missing 裸 INSERT；正式库验证历史股本链游标开始推进、公告 retry 清零。剩余缺口均为披露项与有界续传 | `reports/94`（2026-08-31） |
| 财务明细回填提速与自动完成 | ✅ **完成（2026-08-31）**：B 股 SQL 过滤、并发抓取、批量事务、快速源优先、TDX 预算兜底、缺失出队；实测 2 只/分 → 20.7 只/分；自动更新单周期清空全部缺口队列（10000 上限） | `reports/95`（2026-08-31） |
| 更新过程内存治理 | ✅ **修复完成（2026-09-01）**：正式库 26GB `raw_response_archive` 冷热分层（schema v16：history + 小型 active + 全量读取视图）；三条溯源写路径改为单写者锁内 hash 探测 + 普通 INSERT；财务明细回填 DuckDB 峰值 28GB→3GB、速率 9.8→约29只/分；归档/备份覆盖 history 表 | `reports/96`（2026-09-01） |
| 更新内存上限与吞吐优化 | ✅ **完成（2026-09-01）**：DuckDB 读/写连接统一 `memory_limit=12GB`（冷核对峰值约 13.5GB）；业务概览抓取并发 4（源侧仍 ≤2 req/s），49.7→约60.5只/分；已确认源缺失股票 7 天出队；Sina 财务明细 TEMP TABLE 合并写入 0.17s/只 | `reports/97`（2026-09-01） |
| 更新总耗时压缩 | ✅ **完成（2026-09-01）**：国债曲线变化改用批量 `refresh_treasury_spreads`（5542 只 30s，替代约16分钟全量指标重算）；统计域 worker 复用+曲线缓存+并行12（5553 只约9分钟）；统计域输入指纹改为确定性排序，无真实输入变化时整域跳过；完整更新周期 38→约12分钟；DuckDB 上限调整为14GB | `reports/98`（2026-09-01） |
| 数据库与主机现状调研 | ✅ **完成（2026-09-01）**：DuckDB 49.7GiB 中约16.3GiB为可回收空洞；D盘剩余183.6GB，具备离线重建条件；下一步按 P0 离线重建 → P1 lineage 查询优化 → P2 归档自动轮转 → P3 统计域批量取数 → P4 价格源校准执行 | `reports/99`（2026-09-01） |
| 数据库治理计划风险复审 | ✅ **完成（2026-09-01）**：`reports/99` 调研事实保留、执行计划已修订；P0 增加导出/导入预演与序列校验，P2 改为日历月分区 + 5GB/10万行/31天硬阈值，P3 暂缓 SQL 计算下推、只先做批量取数；新增 P5 可观测性、P6 备份恢复演练 | `reports/100`（2026-09-01） |
| 数据库治理计划第二次复审 | ✅ **完成（2026-09-01）**：方向批准、执行收紧；P0 必须脚本化逐表导出/导入并定义回滚窗口；P1 先交付 SQL 原型；P2 补充 36 分区上限/冷导出清单；P3 冻结黄金数据集；执行前完成当前库新备份与恢复演练 | `reports/101`（2026-09-01） |
| 数据库治理计划执行状态 | ✅ **P0/P1/P2/P3 已实施（2026-09-01）**：49.7GiB 旧库离线重建为约41GB新库并校验通过；schema v17 lineage hash 集与归档分区轮转上线；历史 BLOB 导出 4.9GB Parquet 冷归档；统计域批量预取；旧库/硬链接快照保留可回滚 | `reports/102`（2026-09-01） |
| 技术债补全与个股研究细节体验 | ✅ **完成（2026-09-02）**：T1-T9 评估/补全（冷归档恢复 CLI、分红隔离审计、东财单请求探测、旧规则审计）；重建观察期满足后删除回滚快照与旧备份/归档，释放约 70GB；个股 K 线空白修复、历史统计去重/分位线配色/悬浮提示、目录滚动跟随、自选上市时间与市值排序列 | `reports/103`（2026-09-02） |
...
- **财务明细回填提速与自动完成（2026-08-31，见 `reports/95`）**：回填速度提升约 10 倍；服务重启后自动更新单周期清空全部明细缺口并自动重算快照，无需人工监控。
- **更新过程内存治理（2026-09-01，见 `reports/96`）**：`raw_response_archive` 已完成冷热分层，财务明细回填峰值内存降至约 3GB、速率约 29 只/分；服务已重启加载新 schema 与写入路径。剩余：启动冷核对内存上限与 DuckDB 文件高水位离线压缩评估。
- **更新内存上限与吞吐优化（2026-09-01，见 `reports/97`）**：DuckDB 连接内存上限 12GB、业务概览并发抓取（约 60 只/分）、源缺失股票出队、Sina 财务明细 TEMP TABLE 合并已上线；正式服务正在按新路径续传业务概览缺口。
- **更新总耗时压缩（2026-09-01，见 `reports/98`）**：国债利差批量刷新（30s）、统计域并行 12 + worker 曲线缓存、DuckDB 上限 14GB 已上线；正式库完整更新周期约 12 分钟，业务概览/财务明细缺口已清空。
- **数据库与主机调研及下一步（2026-09-01，见 `reports/99`）**：已确认 16.3GiB 可回收空洞与 183.6GB 空闲磁盘；执行计划已由 `reports/100` 修订为 P6 备份演练 → P0 重建 → P5 监控 → P1 lineage → P2 月分区轮转/冷导出 → P3 批量取数试点。
- **数据库治理计划第二次复审（2026-09-01，见 `reports/101`）**：执行门禁已明确——先做当前库逻辑备份与恢复演练，再脚本化逐表重建；回滚窗口至少一个完整更新周期，观察通过才删除旧文件。
- **数据与代码双重复查（2026-08-31，见 `reports/94`）**：核心数据完整；业务概览 CLI 游标与 backfill missing upsert 修复；财务明细 1000 只加速回填正在运行。
- **更新完成后全量复审计（2026-08-31，见 `reports/93`）**：更新成功；修复历史股本链游标重复队头、国债周末 missing 永久重试、CNINFO 全局 retry 清理、missing 写入冲突；重启后下一轮生效。
- **全局数据路径复审计与指标输入链修复（2026-08-31，见 `reports/92`）**：回购自动调度、融资/国债/retry 提前到快照之前、回购空响应保旧值、内容级曲线指纹、财务明细缺口跨行业探测、CLI 写命令单写者锁。重启服务后生效。
...
| `docs/reports/99_DB_HOST_SURVEY_AND_NEXT_STEPS_2026-09-01.md` | **数据库与主机调研（2026-09-01）**：49.7GiB 文件/16.3GiB 空洞、D盘183.6GB等事实 | 调研事实保留；执行计划已被 `reports/100` 取代 |
| `docs/reports/100_PLAN_RISK_REVIEW_AND_REVISION_2026-09-01.md` | **数据库治理计划风险复审与修订（2026-09-01）**：轮转阈值修正、统计SQL范围收窄、R1-R11风险表 | 风险表与方案修订事实保留；执行计划已被 `reports/101` 收紧 |
| `docs/reports/101_SECOND_PLAN_REVIEW_EXECUTABILITY_2026-09-01.md` | **数据库治理计划第二次复审（2026-09-01）**：条件批准与 Go/No-Go 门禁 | 执行门禁历史依据；执行状态见 `reports/102` |
| `docs/reports/102_PLAN_EXECUTION_STATUS_2026-09-01.md` | **数据库治理执行状态（2026-09-01）**：P0-P3实施、重建校验、回滚快照、冷归档、剩余待办 | 当前执行状态与剩余工作依据 |
| `docs/reports/103_TECH_DEBT_REMEDIATION_AND_UI_POLISH_2026-09-02.md` | **技术债补全与个股研究细节体验修复（2026-09-02）**：T1-T9 补全、约70GB 清理、K线/统计/目录/自选 UI 修复 | 当前技术债与 UI 修复依据 |
