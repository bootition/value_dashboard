...
| 财务明细回填提速与自动完成 | ✅ **完成（2026-08-31）**：B 股 SQL 过滤、并发抓取、批量事务、快速源优先、TDX 预算兜底、缺失出队；实测 2 只/分 → 20.7 只/分；自动更新每轮 500 只，完成器循环清空缺口 | `reports/95`（2026-08-31） |
...
- **财务明细回填提速与自动完成（2026-08-31，见 `reports/95`）**：回填速度提升约 10 倍，服务重启后自动每轮 500 只，完成器循环清空剩余缺口并自动重算快照。
