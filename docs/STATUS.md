...
| 更新总耗时压缩 | ✅ **完成（2026-09-01）**：国债曲线变化改用批量 `refresh_treasury_spreads`（5542 只 30s，替代约16分钟全量指标重算）；统计域 worker 复用+曲线缓存+并行12（5553 只约9分钟）；完整更新周期 38→约12分钟；DuckDB 上限调整为14GB | `reports/98`（2026-09-01） |
