...
7. **东财交叉核验批量风控遗留点（2026-08-12 记录，待后续处理）**：东财 F10 交叉源（`stock_zh_a_gbjg_em`）在批量连续请求约 46-50 次后触发风控；单只/少量请求正常（5 只慢速测试 4/5 交叉成功）。症状：批量回填时交叉核验静默失败（`cross_status=cross_unavailable` 但 `update_stock` 状态仍 success），verified 大量为 false。影响：PE/PB 历史统计因覆盖门槛（只认 verified）大部分为 NULL（如实，不伪造）。已建机制：`capital_cross_cache` 交叉缓存落盘（已核验股票不重跑，杜绝全量重跑）+ 批次审查脚本（`examine_batch.py` 临时版）。**待办**：① `cross_status` 落盘到缓存表（失败也记录原因，批次审查可见）；② 确定"批大小 × 冷却时长"安全组合；③ 批次审查固化进正式流程；④ 交叉核验完成后重建统计域。当前 verified 股票约 56 只。
## 进行中的工作
