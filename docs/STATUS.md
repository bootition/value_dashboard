...
| 全局数据路径复审计与指标输入链 | ✅ **修复完成（2026-08-31）**：回购域接入自动更新；融资/国债/失败重试全部前移到指标快照之前；股本变化与财务 partial 解耦；回购空响应保旧值；国债与统计域指纹升级为内容级；写类 CLI 统一单写者锁。定向回归 105 passed | `reports/92`（2026-08-31） |
...
- **全局数据路径复审计与指标输入链修复（2026-08-31，见 `reports/92`）**：回购自动调度、融资/国债/retry 提前到快照之前、回购空响应保旧值、内容级曲线指纹、CLI 写命令单写者锁。重启服务后生效。
