...
- **最后更新**：2026-09-01
- **更新人**：opencode 会话（2026-09-01 更新内存上限与吞吐优化：`reports/97`）
## 当前裁决（Verdict）
| 层面 | 状态 | 依据 |
|---|---|---|
...
14. **更新内存峰值与启动核对峰值（2026-09-01 更新）**：`raw_response_archive` 新行提交会触发 DuckDB 整表主键校验扫描，已由 `reports/96` 冷热分层修复；DuckDB 连接默认 `memory_limit=12GB` 已由 `reports/97` 落地，启动冷核对峰值降至约 13.5GB 并回落。12GB 对超大分析查询的落盘/耗时影响需在实际使用中观察，可按机器内存经 `config/user.yaml` 调整。
## 进行中的工作
