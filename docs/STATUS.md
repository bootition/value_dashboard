...
- **最后更新**：2026-08-12
- **更新人**：opencode 会话（P3/P4 红队修复完成 + 正式库数据补全）
## 当前裁决（Verdict）
| 层面 | 状态 | 依据 |
|---|---|---|
...
7. **东财交叉核验批量风控遗留点（2026-08-12 记录，待后续处理）**：东财 F10 交叉源（`stock_zh_a_gbjg_em`）在批量连续请求约 46-50 次后触发风控；单只/少量请求正常（5 只慢速测试 4/5 交叉成功）。症状：批量回填时交叉核验静默失败（`cross_status=cross_unavailable` 但 `update_stock` 状态仍 success），verified 大量为 false。**2026-08-12 用户决策：PE/PB 统计先行按 CNINFO 主链口径可用（主链点存在即覆盖，verified 仅披露不阻断）；统计域已按此口径重建（version 3，PE 10y 分位 2,288 只、PB 10y 分位 3,075 只可用）。**已建机制：`capital_cross_cache` 交叉缓存落盘（已核验股票不重跑，杜绝全量重跑）+ 批次审查脚本（`examine_batch.py` 临时版）。**待办**：① `cross_status` 落盘到缓存表（失败也记录原因，批次审查可见）；② 确定"批大小 × 冷却时长"安全组合；③ 批次审查固化进正式流程；④ 交叉核验完成后评估是否收紧回 verified 口径并重建统计域。当前 verified 股票约 52 只。
## 进行中的工作
- **P3/P4 修复已完结（`reports/74`）**：代码/测试/文档全部关闭；正式库国债曲线、快照利差、统计域、价格已补全。
- **CNINFO 股本链续传**：源风控冷却中（2026-08-12 触发，返回非 JSON）；冷却后自动更新有界续传（due 游标 + retry 消费），或手动 `vd data capital-history --max-stocks 1000` 分批加速；完成后统计域按指纹自动重建（PE/PB 覆盖恢复）。
- **自动更新保持 enabled**：服务每轮启动自动执行增量更新；价格已补齐至 2026-08-11。
- **东财交叉核验（遗留，见缺口 #7）**：主链 5,541 只已全量落盘；交叉核验受东财批量风控限制，当前约 56 只 verified；待缺口 #7 待办完成后补全并重建统计域。
- **东财行情源冷却**：push2/push2his 封锁冷却期至 2026-08-15（期间勿触碰）；到期后单次探测，恢复后限速 ≤2 req/s、并发 ≤5。
- **待办**：CNINFO 分红 ex_date 修复评估（见已知缺口 #4）。
- **已完结**：P1-P4 全部实施；P3/P4 系统红队发现全部修复（见 `reports/73/74`）。
