...
## 导入结果（正式库，2026-09-04 实测）

- 命令：`vd data hk-dividends`（全量，0.5s/股源侧限速）。
- 覆盖：映射 202 只（152 精确名 + 50 人工覆写；药明康德 H 02359 因
  A 股端不在当前上市池如实 unmatched）；**179 只成功写入、0 失败、
  0 retry**；23 只源无分红记录如实登记 `missing(hk_dividends/
  source_empty)`（多为近两年新两地上市公司）。
- 数据量：`hk_dividends` 共 **2,587 行**、ex_date 跨度
  **1999-04-30 ~ 2026-12-30**；25 行"方案已公告但除净日未定"
  （ex_date NULL）如实跳过并计数（ex_date 确定后下次导入自然补上）。
- 解析覆盖：dividend_per_share_hkd 2,508/2,587（96.9%）、
  cny 2,418（93.4%），其余特殊方案文本保持 NULL 可追溯原文。
- 抽查 **600941（中国移动）52 行**：2026-08-25
  "每股派人民币2.51元(相当于港币2.9003元)" → hkd=2.9003/cny=2.51 ✓；
  2026-06-05、2025-09-01 等历次派息均正确解析。
- 导入过程实锤并修复一个约束问题：表主键 (stock_code, ex_date,
  plan_explain) 的 ex_date 隐式 NOT NULL，未实施方案行过滤后入库
  （`pending_no_exdate` 报告字段披露）。
