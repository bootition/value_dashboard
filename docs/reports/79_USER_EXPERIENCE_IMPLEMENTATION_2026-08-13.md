...
1. **股息率条件值域口径**：`dividend_yield` 在快照中为小数（如 0.0529 =
   ...
   **已修复（2026-08-13，用户决策）**：`RuleConditionRow` 对 `fieldFormat
   === 'pct'` 的字段（roe/roa/roic/毛利率/净利率/负债率/同比/CAGR/股息率/
   利差等 25 个）做**百分数输入换算**——输入 2 = 2%，底层存 0.02；编辑回显
   时 0.0529 → 5.29；条件值旁显示 "%" 单位与"（百分数，如 2 = 2%）"说明；
   区间条件同样换算。历史统计分位字段（0-100 口径）不受影响。新增组件
   测试 4 项（`rule-condition-row.test.ts`）。
2. exe 发布形态建议：正式分发时把 `data/` 与 exe 同目录放置即可复用现库。

## 四、应用图标与打包形态（2026-08-13 追加）

- 生成原创应用图标 `resources/app-icon.ico`（16~256 多尺寸）：深绿圆角底
  （项目主色系）+ 三根上升 K 线（金/薄荷绿）+ 趋势箭头。造型参考开源
  Lucide（ISC 许可）"candlestick-chart" 元素，配色与构图自绘，无版权风险。
- `value-dashboard.spec` 的 EXE `icon` 指向该 ico；重新构建后 exe 与
  快捷方式将显示新图标。
- 部署说明：exe 必须与 `data/` 同目录（路径隔离策略），桌面使用时需将
  exe 目录与数据一并放置或复制（正式库约 10GB）。
- **用户部署决策（2026-08-13）**：日常使用选定方案 A——桌面快捷方式
  `Value Dashboard.lnk` 指向源模式 `start.bat`（窗口最小化运行、图标为
  `resources/app-icon.ico`），直接复用正式库单一数据源；exe 独立部署
  （方案 B）保留为对外分发用途。
